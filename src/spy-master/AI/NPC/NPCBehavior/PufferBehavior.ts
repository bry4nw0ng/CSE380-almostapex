...
import PinEm from "../NPCActions/PinEm";

export default class PufferBehavior extends NPCBehavior {
    ...
    protected initializeStatuses(): void {
        ...
        this.addStatus(PufferStatuses.CLOSE_ENOUGH, new inRange(this.owner, this.target, 150));
        this.addStatus(PufferStatuses.GOAL, new FalseStatus());
    }
    ...
    protected initializeActions(): void {
        ...
        gitem.addEffect(PufferStatuses.CLOSE_ENOUGH);
        gitem.cost = 1;
        this.addState(PufferActions.GITEM, gitem);

        let shootEnemy = new PinEm(this, this.owner);
        shootEnemy.targets = [this.target];
        shootEnemy.targetFinder = new BasicFinder();
        shootEnemy.addPrecondition(PufferStatuses.CLOSE_ENOUGH);
        shootEnemy.addEffect(PufferStatuses.GOAL);
        shootEnemy.cost = 1;
        this.addState(PufferActions.SHOOT_ENEMY, shootEnemy);

    }

    public override addState(stateName: PufferAction, state: GoapAction): void {
        super.addState(stateName, state);
    }

    public override addStatus(statusName: PufferStatus, status: GoapState): void {
...
export type PufferStatus = typeof PufferStatuses[keyof typeof PufferStatuses];
export const PufferStatuses = {
...
export type PufferAction = typeof PufferActions[keyof typeof PufferActions];
export const PufferActions = {
