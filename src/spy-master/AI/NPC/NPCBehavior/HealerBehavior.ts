...
import { BattlerActiveFilter, BattlerGroupFilter, BattlerHealthFilter, ItemFilter, RangeFilter, VisibleItemFilter } from "../../../GameSystems/Searching/SMFilters";
...
export default class HealerBehavior extends NPCBehavior  {
    ...
    public initializeAI(owner: NPCActor, opts: Record<string, any>): void {
        ...
        this.addStatus(HealerStatuses.HPACK_EXISTS, new TargetExists(scene.getHealthpacks(), new BasicFinder<Item>(null, ItemFilter(Healthpack), (item: Item) => item.inventory === null)));
        ...
        let lowhealthAlly = new BasicFinder<Battler>(null, BattlerActiveFilter(), BattlerGroupFilter([owner.battleGroup]), BattlerHealthFilter(0, owner.maxHealth / 2));
        this.addStatus(HealerStatuses.ALLY_EXISTS, new TargetExists(scene.getBattlers(), lowhealthAlly));
        
        /* ######### Add all healer actions ######## */

        // Action for picking up a healthpack
        let pickupHpack = new PickupItem(this, this.owner);
        pickupHpack.targets = scene.getHealthpacks();
        pickupHpack.targetFinder = new BasicFinder<Item>(ClosestPositioned(this.owner), (item: Item) => item.inventory === null, ItemFilter(Healthpack));
        pickupHpack.addPrecondition(HealerStatuses.HPACK_EXISTS);
        pickupHpack.addEffect(HealerStatuses.HAS_HPACK);
        pickupHpack.cost = 5; // prefer healing over picking healthpack
        this.addState(HealerActions.PICKUP_HPACK, pickupHpack);

        // Action for using a healthpack on an ally
        let useHpack = new UseHealthpack(this, this.owner);
        useHpack.targets = scene.getBattlers();
        useHpack.targetFinder = new BasicFinder<Battler>(ClosestPositioned(this.owner), BattlerActiveFilter(), BattlerGroupFilter([owner.battleGroup]), BattlerHealthFilter(0, owner.maxHealth / 2), (b: Battler) => b !== owner);
        ...
        useHpack.cost = 1; // prefer healing over picking healthpack
